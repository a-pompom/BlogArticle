...
# 関数定義
# 誕生日・現在日付を入力に、年齢を出力
def calculate_age(birthday: str, current_date: datetime) -> int:
    # 年齢を算出
    age: int = 年齢算出処理()
...
ここで注目すべきは、年齢を計算する関数が基準となる日付をどのように参照しているか、というところです。
...
結論から言うと、プライベートメソッドが複雑になってきてテストコードを書きたくなったら、別のクラスへ切り出すべき合図だと思った方がシンプルに考えられます。
...
それでは、なぜ検証することがassertionと呼ばれるのでしょうか。
